...
import {
  ...
  type WaitlistEmailRole,
} from "./emailTemplate";

export type { WaitlistEmailRole };

type Payload = { toEmail: string; verifyLink: string; name?: string | null; role?: WaitlistEmailRole | null };
type WelcomePayload = { toEmail: string; dashboardLink: string; name?: string | null; role?: WaitlistEmailRole | null };
...
async function sendWelcomeViaResend(payload: WelcomePayload, displayName: string): Promise<DeliveryResult> {
  const apiKey = process.env.RESEND_API_KEY;
  const from = getFrom();
  const replyTo = process.env.WAITLIST_EMAIL_REPLY_TO;
  if (!apiKey || !from) return { delivered: false, provider: "resend", error: "Resend is not configured." };
  ...
  try {
    const response = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        html: buildWaitlistWelcomeHtml({ dashboardLink: payload.dashboardLink, displayName, role: payload.role }),
        text: buildWaitlistWelcomeText({ dashboardLink: payload.dashboardLink, displayName, role: payload.role }),
        ...(replyTo ? { reply_to: replyTo } : {}),
      }),
      cache: "no-store",
    });
    ...
    return { delivered: false, provider: "resend", error: error instanceof Error ? error.message : "Welcome email delivery failed." };
  }
}

async function sendWelcomeViaWebhook(payload: WelcomePayload, displayName: string): Promise<DeliveryResult> {
  const webhookUrl = process.env.WAITLIST_EMAIL_WEBHOOK_URL;
  if (!webhookUrl) return { delivered: false, provider: "webhook", error: "Webhook not configured." };
  ...
  try {
    ...
    return { delivered: false, provider: "webhook", error: error instanceof Error ? error.message : "Welcome webhook delivery failed." };
  }
}

export async function dispatchWaitlistWelcomeEmail(payload: WelcomePayload): Promise<DeliveryResult> {
  const displayName = displayNameFor(payload.toEmail, payload.name);
  const resendResult = await sendWelcomeViaResend(payload, displayName);
  if (resendResult.delivered) {
    logger.info({
      scope: "notifications.waitlistWelcome",
      message: "Welcome email accepted by provider.",
      data: { provider: resendResult.provider, recipientDomain: emailDomain(payload.toEmail) },
    });
    return resendResult;
  }

  const webhookResult = await sendWelcomeViaWebhook(payload, displayName);
  if (webhookResult.delivered) {
    logger.info({
      scope: "notifications.waitlistWelcome",
      message: "Welcome email accepted by provider.",
  ...
  if (process.env.NODE_ENV === "production") {
    await logger.captureError({
      scope: "notifications.waitlistWelcome",
      message: "Waitlist welcome delivery failed in production.",
      data: {
        recipientDomain: emailDomain(payload.toEmail),
        resendError: resendResult.error,
        webhookError: webhookResult.error,
    ...
    return { delivered: false, provider: "webhook", error: "Welcome email delivery failed." };
  }

  logger.warn({
    scope: "notifications.waitlistWelcome",
    message: "Welcome email used the development console fallback.",
    data: { recipientDomain: emailDomain(payload.toEmail), role: payload.role ?? "FOUNDER" },
  });
  return { delivered: true, provider: "console" };
}
